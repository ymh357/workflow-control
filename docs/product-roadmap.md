...
| Single-session 模式 | **Won't-fix — closed 2026-04-25** | kernel-next 当前仅 multi-session（Stage 4a 删除了 legacy SessionManager）。决策门原本是"vs legacy R1 的 3-5× 差距若不能缩到 ≤1.5× 则回补"——但 legacy R1 已随 Stage 4a 一起退役，benchmark 对照组不存在。同时 2026-04-25 的 live probe（详见 `docs/superpowers/plans/2026-04-24-linear-dogfood-followup-and-D-prime-done-handoff.md` §17）证实 Claude CLI（SDK v0.2.63 bundled `cli.js`）**自动启用 prompt caching**（ephemeral 1h TTL，warm-vs-cold cost 下降 ~5×），无需 SDK 层 opt-in。回补的核心动机（repeated system-prompt token 消耗）由 auto-caching 自发解决。未来如果 per-attempt 的 cache hit rate（现已持久化到 `agent_execution_details.cache_read_input_tokens` / `cache_creation_input_tokens`，见 §17.3）或 per-stage cost 显示问题，可凭新证据重开本决策；目前零证据支撑回补。B10 / B12 相关条目一并从 pending 移到 closed。|
...
| B12 | **Single-session 热更新**：原设计是开新 session + 旧对话历史以摘要形式注入 tier1。Won't-fix — closed 2026-04-25 随 single-session 决策一起关闭（§4 行 105）。Multi-session 架构下原本就不适用（每个 stage 本就是新 session）；等价能力由 port-level summary handoff 提供（B10 graceful summary turn 已落地：agent 被 interrupt 前写 summary 到声明的 output port，下游 stage 读该 port，hot-update/migration.graceful-summary.test.ts 覆盖） |
...
- Single-session：Won't-fix — closed 2026-04-25（§4 行 105）。Claude CLI 自动启用 prompt caching + legacy R1 基线不存在，回补触发条件作废
- 白皮书：暂停维护
...
- B11 不相关 stage：跑完再切换
- B12 Single-session：Won't-fix — closed 2026-04-25 随 §4 行 105 决策一起；等价能力由 B10 graceful summary turn（multi-session 原生机制）覆盖
...
| 2026-04-25 | 1.18 | Single-session 回补决策关闭 + prompt caching 诊断落地：context7 + Anthropic docs + SDK 源码 + live probe 四路证据证实 Claude CLI (SDK v0.2.63 bundled `cli.js`) 自动启用 prompt caching（ephemeral 1h TTL，warm-vs-cold ~5× 成本下降）。`agent_execution_details` 新增 `cache_read_input_tokens` + `cache_creation_input_tokens` 两列，real-executor 从 SDK `result.usage` 捕获并写入（writer.close / flushNow 双 UPDATE 同步更新）；+2 writer tests，+2 schema PRAGMA assertions，1052 tests 全绿。原 §4 行 105 决策门因 legacy R1 基线退役而作废，single-session 回补 / B12 正式标 Won't-fix。完整证据链：`docs/superpowers/plans/2026-04-24-linear-dogfood-followup-and-D-prime-done-handoff.md` §17。|
