...
| Single-session 模式 | **TODO / 决策 open — deferred 2026-04-25** | kernel-next 当前仅 multi-session（Stage 4a 删除了 legacy SessionManager）。原决策门"vs legacy R1 的 3-5× 差距若不能缩到 ≤1.5× 则回补"对照组已不存在（R1 随 Stage 4a 退役），需要**重新定义决策门**。2026-04-25 的初步证据（详见 `docs/superpowers/plans/2026-04-24-linear-dogfood-followup-and-D-prime-done-handoff.md` §17）仅涵盖"同 prompt 反复调用" within-session cache hit，不能直接推导出"多 stage pipeline 中共享的 claude_code preset 是否被复用"——两者是不同 use case。决策条件重新制定前不关闭。已落地的 observability（`agent_execution_details.cache_read_input_tokens` / `cache_creation_input_tokens`，见 §17.3）是凭据来源。相关：B10 Graceful summary turn 已落地（`multi-session` 下等价能力）；B12 仍依赖本决策。|
...
| B12 | **Single-session 热更新**：开新 session。旧对话历史以摘要形式注入新 session 的 tier1 —— 依赖 single-session 回补（§4 行 105 TODO / deferred）。Multi-session 架构下不适用（每个 stage 本就是新 session）；等价能力由 B10 graceful summary turn 提供（agent 被 interrupt 前写 summary 到声明的 output port，下游 stage 读该 port，`hot-update/migration.graceful-summary.test.ts` 覆盖）。本条状态随 §4 行 105 决策联动 |
...
- Single-session：TODO / deferred 2026-04-25。legacy R1 基线已退役，原决策门 "3-5× → ≤1.5×" 无对照；2026-04-25 初步证据只覆盖 within-session cache hit，不能直接回答 multi-stage 场景。决策 open，等待多 stage 实测数据
- 白皮书：暂停维护
...
- B11 不相关 stage：跑完再切换
- B12 Single-session：跟 §4 行 105 TODO / deferred 联动；multi-session 架构下等价能力由 B10 graceful summary turn 提供，但正式决策仍 open
