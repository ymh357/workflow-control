...
| # | 决策 |
|---|---|
| B9 | **Worktree 切换**：git reset 到改动 stage 的 checkpoint + 旧 diff 写进 StageMemory 作参考 ⚠️ 部分落地 Phase 4.5 Tier2：新 `migration_hints` 表捕捉被 supersede attempt 的 diff，RealStageExecutor consume 后注入新 attempt system prompt 作为"Migration note" advisory。**Phase 5C worktree ownership 契约 2026-04-23 落地** (`task_worktrees` 表 + `allocateWorktree` idempotent per-taskId + `startPipelineRun` opt-in integration + git worktree add/list/remove 包装)，B9 full（git reset --hard before_sha 在 owned workdir 内）结构已解锁，留待独立 PR 实施。多 task 共享 workdir 已不再是阻塞因素。 |
