...
在 single-session 模式下，tier1Context 会在每个 stage 的用户消息中重新注入（而不仅仅是第一个 stage）。SDK 自动压缩触发后，早期 stage 的原始 tier1 会被摘要化——重新注入确保 Agent 无论压缩历史如何，始终拥有完整、最新的任务上下文。
...
### 8.4 Single-Session 压缩优化

在 single-session 模式下，所有 Stage 共享同一个 Claude Agent SDK 会话，对话历史跨 Stage 无限增长。SDK 的自动压缩机制在约 167k tokens 时触发（200k 上下文窗口），使用通用摘要 prompt 对早期对话进行压缩。

为防止通用压缩丢失 Pipeline 关键信息，系统注册了 `PreCompact` hook，在压缩 LLM 执行前注入 pipeline 感知的指令，告知压缩 LLM 必须保留以下内容：
- Store 键值对（当前快照）
- Scratch pad 笔记（Agent 自维护的工作记忆）
- 已完成 Stage 的进程与结果摘要
- 文件修改上下文（路径、变更原因）
- 影响下游 Stage 的关键决策

Hook 读取 `SessionManager` 的可变字段（`currentStageName`、`currentContext`、`currentScratchPad`），这些字段在每次 `executeStage()` 开始时同步更新，确保压缩指令始终反映当前 Stage 的状态。

此外，store reader MCP（`__store__`）在每次 Stage 切换时通过 `switchStageConfig()` 无条件刷新，确保 Agent 始终看到最新的 Store 数据。SDK 的 `setMcpServers` 是 diff-based 的，未变化的外部 MCP 服务器不会被重连，避免不必要的连接开销。

压缩边界的 `compact_boundary` 系统消息被观测并转发为 SSE 事件，用于调试和可观测性，使运维人员能够追踪压缩发生的时机和上下文。
...
| `compact` | 压缩边界检测（触发器、压缩前 token 数、stage） | Session 管理器（compact_boundary 系统消息） |
...
#### Gemini 成本追踪采用估算
Gemini CLI 不原生报告成本数据。`estimateGeminiCost()` 提供三级回退：(1) 直接使用 `stats.cost_usd`（如果 CLI 报告了），(2) 聚合 `stats.models` 中的 per-model cost，(3) 基于 token 数按 Flash/Pro 分别定价估算。Codex 采用类似的 token 估算方案。

**影响：混合引擎 Pipeline 现有近似成本核算。准确性取决于定价表的时效性。**
...
- SDK PreCompact hook 集成实现了 pipeline 感知的对话压缩，在自动压缩边界跨越时保留关键 stage 上下文
...
*本文档反映 2026-04-18 代码库状态。所有声明均基于源码逐行分析，缺陷章节经过完整验证。*
