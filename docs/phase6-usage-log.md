...
- **M3 分子/分母**: 14 / 21（run #2,3,4,9,11,12,14,15,16,17,18,19,20,21 真 completed；run #1,5,6,7,10,13 API completed 但未全跑 / 内容错；#8 是 B5 API 验证）
- **真实成功率**: 14/21 = **67%**。架构审计修完后的 runs 全过
- **post-architecture-audit 子集**: 10/10 = **100%**（runs #11,#12,#14,#15,#16,#17,#18,#19,#20,#21）
