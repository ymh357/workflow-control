...
| 28 | 2026-04-23 | **PG complex-pipeline regression**（re-run #19 with all debts fixed）| Pipeline Generator → **GitHub PR Security Review** (new hash `7798eda1`) | `pipeline-generator-1776954840989-4bc6d1f3` | completed ✅ + real versionHash in DB | ~8 min, $0.3383 | — | 重现 run #19 原始任务（"GitHub PR → security review"，带 GitHub MCP 依赖 + inline object `prMetadata` type）验证 debts L/M/N/O/T 联合是否修好。**结果**：persisting stage 真 submit 成功，写入 `versionHash=7798eda1...`（真 SHA）/ `pipelineId=github-pr-security-review`（真 slug），生成的 pipeline 入 `pipeline_versions` 表。**对比 run #19 (pre-fix)**：当时 persisting 看到 bogus WIRE_TYPE_MISMATCH，写 FAILED sentinel，pipeline 不入 DB。IR 质量 audit：两端 inline object type 字符一致（tsc 通过）；3 store_schema entries 覆盖完整；prompt 里 kernel MCP tool name 100% 正确形式（7 处全 `mcp____kernel_next____`），fetchDiff prompt 4 处引用 GitHub MCP。debt L/M/N/O/T 联合验证通过。只差真跑生成的 pipeline（需 GitHub MCP 实配 + 真 PR 编号）|
| 27 | 2026-04-23 | **Generated pipeline end-to-end run（3x）** | 3 个 PG 输出的 URL→hostname pipeline 各跑一次 | `url-hostname-extractor-1776954307986-15ff99e8` + 2 其他 | **3/3 completed/natural ✅** | ~30s each | — | 3 个 PG 生成的 pipeline 都用相同 input `https://api.subdomain.example.org:8443/v1/users?foo=bar#section` 跑一次。**全部输出正确 `api.subdomain.example.org`**。**每个 pipeline 只调 1 次 write_port**（vs run #22 Writer double-call 2 次，debt O + T 联合闭环修好）。tool_calls_json.isError 全 False，result.ok=true。AI-generated pipeline 真实可用率：5/5 = 100%（含 Research Report Gen run #17 + Extract Hostname run #26 + 3 个 URL Hostname 变体）|
| 26 | 2026-04-23 | **Extract Hostname end-to-end run**（run #20 生成的 pipeline 首次真跑）| `Extract Hostname` (`52d3b767`) | `extract-hostname-1776953934889-83783abd` | completed/natural ✅ | ~30s | — | Input: `https://www.github.com/anthropics/claude-code/issues/123?ref=foo`。Output: `hostname="www.github.com"` ✅ 正确。run #20 生成的 pipeline 首次端到端验证 |
| 25 | 2026-04-23 | **PG 3x stability suite**（同任务 URL→hostname） | 3 次独立 PG run | `pipeline-generator-1776953997943-54e29ccc` + 2 其他 | **3/3 completed/natural ✅** | $0.6940 total | **Debt T: 生成的 prompt MCP tool name form 未规范** | **Investigate**：任务 "URL → hostname"。3 次 PG run：(1) 100% 成功，(2) 3 个生成的 IR **结构 100% 一致**（1 stage agent, url:string→hostname:string, 1 wire, 1 store_schema entry）, (3) pipelineName 2 变体 vs 1 (mostly stable)。**审生成的 prompt 发现 debt T**：`gen-prompts.md` line 39 "Include literal write_port example" 没指定 MCP tool name form —— downstream prompt-writer sub-agent 靠 SDK 训练数据猜，容易输出 `mcp__kernel_next__write_port`（2-underscore 错误形式）。Fix commit `fd8c0a1`：gen-prompts.md 明确列出 5 个 kernel MCP tool 的正确四下划线形式。Re-run 3x：**100% 正确 `mcp____kernel_next____` 形式**，debt T 闭环 |
| 24 | 2026-04-23 | **M4 rollback full-path dogfood**（rollback route 新增 + IR-level migrate+rollback） | smoke-test → v2 `574c4379` → v3 `086720ea` | `smoke-test-1776952126183-22ddfffd` | rollback completed ✅ | instant | — | **补全 M4 最后一条路径**。先跑 smoke-test 完成 → propose 1（prompt-only，autoApprove+migrate）→ HTTP migrate 成功（rerunFrom=greet, supersededStages=[echoBack, greet]）→ 再 propose 2（IR-level update_stage_config 改 promptRef, autoApprove+migrate）→ HTTP migrate → **HTTP rollback 到 v2 成功**：divergenceStage='greet', rolled_back audit row 入 DB。**端到端证明**：M4 rollback 路径在 HTTP 层、orchestrator 层、DB audit 层全 work。**副产物**：新增 `POST /api/kernel/tasks/:taskId/rollback` HTTP 路由（commit `4890c33`）——之前只有 MCP surface。**M4 指标**：propose/approve 8 次 / reject 1 次 / rollback 1 次 |
| 23 | 2026-04-23 | **M4 reject-path dogfood** | Pipeline Generator propose (prompt-only) | `proposal_id=56b03d1d-9684-4bda-adfb-70fd1154613f` | rejected ✅ | instant | — | HTTP propose → `proposalId=56b03d1d`, `proposedVersion=8d66228158cc` → HTTP `POST /api/kernel/proposals/:id/reject` with reason → DB `pipeline_proposals.status='rejected'` 验证。M4 reject 路径首次真实覆盖 |
...
- **M3 分子/分母**: 19 / 26（新增 run #25 3x PG + run #26 Extract Hostname + run #27 3x 生成 pipeline run + run #28 PG complex regression，全 completed/natural；runs #23/#24 propose/rollback 流程验证不计）
- **真实成功率**: 19/26 = **73%**
- **post-architecture-audit 子集**: 15/15 = **100%**
- **AI-generated pipeline 真实 end-to-end 跑通率**: **5/5 = 100%**（Research Report Gen #17 + Extract Hostname + 3 个 URL Hostname 变体；复杂 GitHub PR Security Review 已生成入 DB 但未真跑——需 GitHub MCP 实配）
...
- **M4 热更新 propose / reject / rollback**: **8 / 1 / 1**（run #23 reject, run #24 rollback 完整路径首次覆盖）
