...
2. ✅ tech-research-collector 浏览器 E2E 验证通过（taskId `trc-full-1776675382`，2026-04-20 08:56-08:58）：
   - POST 返回 202 + versionHash `7810b89730a546ca25f391428a3e0d705bb4083b6bf6dec9fe1224fe09e52afa`
   - SSE 事件流完整（`/api/kernel-next/tasks/<id>/stream`）：
     - `task_state × 3`：idle → running → completed
     - `stage_executing × 1`：collectTargetSources
     - `port_written × 10`：2 seed（pipelineConfig, projectContext）+ 8 stage outputs
     - `stage_done × 1`：collectTargetSources
     - `run_final: { finalState: "completed", stageErrors: [] }`
   - 参数：`maxTurns: 30`, `maxBudgetUsd: 1.5`, Claude SDK real executor
   - 耗时：~90s
3. ✅ 既有测试全绿：4128 passed，0 regression。已有 272 test files 都没红
4. ✅ `pnpm tsc --noEmit` 0 errors。既有 adversarial tests 零削弱

### Task 3.2 发现的一个 dev 陷阱（非本 milestone bug）

执行浏览器验证时发现：**既有 `/tmp/workflow-control-data/kernel-next.db` 在 dev 环境持久化**，Task 1.1 的 CHECK 约束扩展只在新建 db 时生效。`initKernelNextSchema` 用 `CREATE TABLE IF NOT EXISTS`，老 db 的老 CHECK 会吞掉新的 `external` kind。

**Workaround**：第一次 dev 运行本 milestone 前需要 `rm /tmp/workflow-control-data/kernel-next.db`。

spec §4.8 已明说不写 migration 脚本（"schema recreated from `initKernelNextSchema`"）——在 spike/dev 阶段可接受，但如果本 milestone 上线到长期运行环境需要一次性 migration。**暂归 §1.3 deep live-migration 范围**。
...
**Task 3.2 浏览器 E2E 验证：已在 2026-04-20 完成**（见 §4 item 2）。重放命令（需先删 `/tmp/workflow-control-data/kernel-next.db`）：
```bash
cd /Users/minghao/workflow-control/apps/server && pnpm dev  # port 3001
cd /Users/minghao/workflow-control/apps/web && pnpm dev     # port 3004
curl -X POST http://localhost:3001/api/kernel/tasks/run \
  -H 'Content-Type: application/json' \
  -d '{
    "pipeline": "tech-research-collector",
    "taskId": "trc-verify-'$(date +%s)'",
    "seedValues": {
      "pipelineConfig": {"targetName": "vitest"},
      "projectContext": {"repoRoot": "/tmp"}
    },
    "maxTurns": 30, "maxBudgetUsd": 1.5
  }'
# 浏览器：http://localhost:3004/kernel-next/<taskId>
# 或 curl SSE: http://localhost:3001/api/kernel-next/tasks/<taskId>/stream
```
