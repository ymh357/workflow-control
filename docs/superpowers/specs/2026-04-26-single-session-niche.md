...
2. ✅ **Run a true niche-internal A/B experiment** (DONE 2026-04-26). 6 variants (2 scenarios × 3 modes) ran end-to-end against `claude-haiku-4-5`. Per-run metadata, port outputs, scoring JSON, and the filled-in results-template all live under `docs/superpowers/specs/single-session-niche-experiment/results/`. Total experimental cost ≈ $0.85 USD (well under the $5-15 budget); total wall-clock ≈ 14 minutes of model time + ~30 minutes of orchestration. Two independent Sonnet reviewers scored each output; one R3 tie-break review resolved a 6-point disagreement on `a3_antiHallucination` for explore-propose. Decision per `protocol.md §5` decision tree (strict): **NICHE NARROWER THAN SPEC** — see step 3 for the verdict and downstream consequences.

3. ✅ **Resume this spec with empirical data** (DONE 2026-04-26). Verdict: NICHE NARROWER. The runtime feature (cross-segment-resume) functions correctly — both single-mode runs share their stage's sessionId, no data loss, no errors. But the *generator-level* claim ("single is universally better when working state isn't easily structurable") is **not** universally supported by evidence:

   - **explore-propose**: single beats multi by **+5 quality points** (49.0 vs 44.0), at parity cost (+0.2%) and 15% faster wall-clock. The mechanism is identifiable: multi-mode's typed-port handoff (`filesConsidered`, `filesRuledOut`, `couplingObservations`, `summary`) loses verbatim function names, which the proposer then *invents plausibly* — `startStreamPump` and `getRealExecutorSdkOptions`, both wrong (actual: `pumpSdkStream`, `buildSdkBaseOptions`). Single-mode's inherited conversation history preserves the symbols. **Niche vindicated** for codebase-exploration / archaeology tasks.
   - **propose-critique**: single LOSES to multi by **−5 quality points** (44.5 vs 49.5), at lower cost (−32%) and faster wall-clock (−20%). The mechanism is opposite: multi-mode's structural separation forces the critic to engage with the proposer's design at face-value, producing a more disciplined CRITICAL/MAJOR/MINOR/NIT calibration (multi 8.0, single 6.0); single-mode's inherited proposer-confidence contaminates the critic, producing softer severity labels and fewer original stress-test scenarios (multi 9.0, single 7.0). **Niche falsified** for design+critique tasks: stage isolation here is a *feature*, not an overhead.

   **Action items**:

   - Tighten §2 criteria. The structurability test (§4) is necessary but not sufficient: a second criterion must distinguish "stages where conversation context helps the next stage" from "stages where structural separation helps the next stage". The propose-critique result identifies the latter class explicitly: when stage-2 needs *adversarial distance* from stage-1, single mode actively hurts. Update §2 to list "the second stage benefits from independent re-examination of stage 1's conclusions" as a disqualifying criterion (§3 territory).
   - Pipeline-generator gate stays in place. The generator should NOT emit `session_mode: "single"` based on 2-stage shape alone. A future iteration may emit it after detecting "exploration / archaeology" patterns specifically (e.g., stage 1 reads files and stage 2 writes recommendations), but this requires a third niche-positive scenario to confirm the narrow case generalises.
   - The §7.1 / §7.2 placeholder performance bounds become empirically anchored: single's wall-clock is 0.80–0.85× multi's (well within the placeholder ≤1.10× ceiling); cost is 0.68–1.00× multi's (within ≤1.20× ceiling). The performance contract holds; the *quality* contract is conditional on scenario.
   - The §10.3 circular-reasoning concern is addressed: the empirical results split with identifiable mechanisms in *both* directions, not one. This is what a non-circular measurement looks like.

   **Step 3 status**: spec sections requiring update:
   - §2 needs a new disqualifying criterion (adversarial-distance requirement).
   - §3 absorbs the new criterion as an explicit case.
   - §7.1 / §7.2 update placeholder numbers with the empirical 0.80–1.00× cost / 0.80–0.85× wall-clock observed; flag the quality contract as scenario-conditional.
   - §10.1 (paradigm vs optimization): the data lands in the middle. It is a *paradigm* in the explore-propose sub-case (the niche has positive expected value) and an *anti-paradigm* in propose-critique (multi+ports has positive expected value). Performance is competitive in both. Frame §10.1 as "context-dependent paradigm".

   These spec edits can land in a follow-up; this resolution closes step 3's "blocked on step 2" status.

**Current state (2026-04-26 post-experiment)**: all three steps are closed. The verdict is NICHE NARROWER. pipeline-generator continues to NOT emit `session_mode: "single"` because the empirical case for *automatic* selection is split — single helps on codebase exploration but hurts on design+critique. The runtime feature stays compiled in and is exercised by the experiment IRs; future scenario-aware generator heuristics can opt in once a third niche-positive scenario confirms the narrow case generalises beyond exploration.

**Why the gate stays in place after the experiment**: the experiment showed the runtime feature works correctly *and* that its quality benefit is scenario-dependent. Lifting the generator gate would require emitting `session_mode: "single"` only for detected exploration/archaeology stage shapes — that detection logic does not yet exist, so the safest default is to keep emitting `multi` until it does.

**Recommended next session shape** (when someone iterates):
- Re-read `docs/superpowers/specs/single-session-niche-experiment/results-template.md` for the verdict and per-dimension breakdown.
- If pursuing wider single-mode adoption: pick a third scenario from §4 (one of the §4 examples NOT yet tested — e.g. open-ended-research synthesis) and run it through the same protocol. If it lands single-positive, generalise the niche to "exploration-class tasks". If it lands single-negative, the niche is narrower than even today's verdict suggests and probably needs to be retired.
- If editing this spec: update §2/§3/§7/§10.1 per the action items above. The post-experiment numbers are the authoritative source for §7's bounds.
