...
export class KernelNextBroadcaster {
   ...
   * synchronously for every event in the existing history (in order),
   * and then for every subsequent `publish` call until unsubscribe.
   * Returns an `unsubscribe` function that is idempotent.
   */
  subscribe(taskId: string, listener: KernelNextSSEListener): () => void {
  ...
  /**
   * Publish an event. Always appends to the task's history ring,
   * then dispatches to current subscribers. Listener errors are
   * swallowed — a broken consumer must not stop the runner.
   */
  ...
  private ensureChannel(taskId: string): TaskChannel {
    ...
    if (!channel) {
      channel = { listeners: new Set(), history: [] };
