...
function buildSubmitSystemPrompt(): string {
  return [
    ...
    `  \`${SUBMIT_TOOL}\`.`,
    "",
    "### PipelineIR shape (terminal design)",
    "StageIR is a **discriminated union** keyed by `type`. Each variant",
    "takes a different `config` shape. Do not mix fields across variants.",
    "```",
    "{",
    "  name: string,                              // pipeline name",
    "  stages: Array<StageIR>,                    // see union below",
    "  wires: Array<{",
    "    from: { stage: string, port: string },",
    "    to:   { stage: string, port: string },",
    "    guard?: string                           // optional runtime expr",
    "  }>",
    "}",
    "",
    "// StageIR variants:",
    ...
    "  config: { promptRef: string },             // resolved by userland",
    "  fanout?: { input: string }                 // optional per-element",
    "}",
    "| {",
    "  name: string, type: 'script',",
    "  inputs: ..., outputs: ...,",
    "  config: { moduleId: string },              // userland script module",
    "  fanout?: { input: string }",
    "}",
    "| {",
    "  name: string, type: 'gate',",
    "  inputs: ..., outputs: ...,",
    "  config: {",
    "    question: { text: string, options?: string[] },",
    "    routing:  { routes: Record<string, string> } // answer -> stage",
    "  }",
    ...
    "6. Gate stages: all values in `config.routing.routes` must be names of",
    "   declared stages. The special key '_default' matches unlisted answers.",
    "7. Fanout: if a stage declares `fanout.input`, it must name one of the",
    "   stage's own input ports (which the wire delivers as T[]); the",
    "   kernel spawns one instance per element.",
    "",
    "### Output protocol",
    ...
    "  - GATE_FANOUT_FORBIDDEN: remove fanout from the gate stage",
...
function buildPatchSystemPrompt(baseVersion: string, baseIrJson: string): string {
  return [
    ...
    "        // keys must be allowed for the target stage variant:",
    "        //   agent  -> { promptRef }",
    "        //   script -> { moduleId }",
    ...
    "   identifiers, unique names, DAG, type-compatible wires, gate",
    "   routing targets exist, and fanout.input names a real input port.",
    "4. StageIR is a discriminated union on `type`; `update_stage_config`",
    "   rejects keys that the target stage's variant does not allow.",
    "",
    "### Output protocol",
    `  ${PROPOSE_TOOL}({`,
    `    currentVersion: "${baseVersion}",`,
