...
- The input port carrying the user's task description is named `taskText` on this stage (see §Inputs above). Read it from your prompt's input section directly — it is already provided to you as a string variable. Do NOT call `read_port` to fetch `taskDescription` / `taskText` / `description` from any stage; those calls return `port_not_declared` because the seed is on the synthetic `__external__` stage, not on a stage you can name. If your prompt input section shows `taskText` empty AND you have already read the input section (not skipped it), only then emit the placeholder path: `pipelineName="unknown"`, `pipelineId="unknown"`, `assumptions=["Task description was empty; produced placeholder design."]`, and a `stageDesign` that explains the gap. Triggering this path because of a mistaken `read_port` call is a hard contract violation — re-read your inputs.
